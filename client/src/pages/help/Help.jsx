...
export function TechTooltip({ term, tech, simple, children, className }) {
  ...
  return (
    ...
    >
      {children || term}
      {show && (
        <span className="absolute bottom-full left-1/2 z-50 mb-2 w-72 -translate-x-1/2 rounded-xl border border-slate-700 bg-slate-950 p-3 shadow-2xl text-left pointer-events-none transition duration-150 animate-in fade-in">
          <span className="block font-mono text-[10px] font-bold uppercase tracking-wider text-cyan-400 mb-1 flex items-center gap-1">
            <HelpCircle className="h-3 w-3" /> Technical & Simple Explanation
          </span>
          <span className="block text-xs font-semibold text-white mb-1">
            <span className="text-cyan-300 font-mono">Technical: </span>{tech}
          </span>
          <span className="block text-xs text-slate-300 border-t border-slate-800 pt-1 mt-1">
            <span className="text-emerald-400 font-semibold">Simple: </span>{simple}
          </span>
        </span>
      )}
    </span>
      ...
      content: (
        <div className="space-y-4 text-xs text-slate-300">
          <p className="leading-relaxed">
            In the <strong className="text-white font-mono">Projects</strong> page, you register the target server endpoint you want to test.
          </p>
          <ul className="space-y-2 list-disc pl-4 text-slate-300">
            <li>
              <strong className="text-cyan-300">HTTP Method Selection:</strong> Choose between <code className="text-cyan-400 font-mono">GET</code>, <code className="text-emerald-400 font-mono">POST</code>, <code className="text-amber-400 font-mono">PUT</code>, <code className="text-purple-400 font-mono">PATCH</code>, or <code className="text-red-400 font-mono">DELETE</code>.
            </li>
            <li>
              <strong className="text-cyan-300">JSON Payload Textarea:</strong> Provide request body data (e.g. <code className="text-slate-400 font-mono">{"{\"username\":\"admin\"}"}</code>) for scanning POST/PUT APIs.
            </li>
            <li>
              <strong className="text-cyan-300">Live Sparkline Analytics:</strong> Watch historical vulnerability counts and request latency trends on interactive SVG curves.
            </li>
          </ul>

          <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 font-mono text-xs space-y-2 mt-2">
            <div className="flex items-center justify-between text-[10px] text-slate-400 border-b border-slate-800 pb-2">
              ...
              <span className="text-cyan-400">POST /api/v1/auth/login</span>
            </div>
            <div className="space-y-1.5 text-slate-300">
              <span className="text-slate-500 block">// Selected HTTP Method:</span>
              <span className="inline-block rounded bg-emerald-500/20 px-2 py-0.5 font-bold text-emerald-300 border border-emerald-500/30">
                POST
              </span>
              <span className="text-slate-500 block pt-2">// JSON Request Body Payload:</span>
              <pre className="text-cyan-300 bg-slate-950 p-2.5 rounded border border-slate-800">
{`{
  ...
  "role": "ADMIN"
        ...
        <div className="space-y-4 text-xs text-slate-300">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            <div className="p-3 rounded-lg border border-slate-800 bg-slate-950">
              ...
              />
              <p className="text-[11px] text-slate-400 mt-1">App safety rating from 0 to 100.</p>
            </div>
            ...
            <div className="p-3 rounded-lg border border-slate-800 bg-slate-950">
              ...
              />
              <p className="text-[11px] text-slate-400 mt-1">Standard severity score used worldwide.</p>
            </div>
          </div>

          <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 text-xs space-y-3 mt-2">
            <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-slate-400 block border-b border-slate-800 pb-2">
              CVSS 3.1 SEVERITY SCALE BREAKDOWN
            </span>
            <div className="space-y-2 font-mono text-[11px]">
              <div className="flex justify-between items-center bg-red-500/10 p-2 rounded border border-red-500/30 text-red-300">
                <span>Critical Risk (9.0 – 10.0)</span>
                <span className="font-bold">Immediate Fix Needed</span>
              </div>
              <div className="flex justify-between items-center bg-orange-500/10 p-2 rounded border border-orange-500/30 text-orange-300">
                <span>High Risk (7.0 – 8.9)</span>
                <span className="font-bold">Fix within 48 Hours</span>
              </div>
              <div className="flex justify-between items-center bg-amber-500/10 p-2 rounded border border-amber-500/30 text-amber-300">
                <span>Medium Risk (4.0 – 6.9)</span>
                <span className="font-bold">Fix within 7 Days</span>
              </div>
              <div className="flex justify-between items-center bg-emerald-500/10 p-2 rounded border border-emerald-500/30 text-emerald-300">
    ...
    3: {
      ...
      content: (
        <div className="space-y-4 text-xs text-slate-300">
          <p className="leading-relaxed">
            The <strong className="text-white font-mono">Findings</strong> database lists every discovered vulnerability with CWE classification, OWASP category, target endpoint, and SLA countdown timers.
          </p>
          <ul className="space-y-2 list-disc pl-4 text-slate-300">
            <li>
              <TechTooltip
            ...
            <li>
              <strong className="text-cyan-300">Captured Technical Evidence:</strong> Displays raw HTTP status codes (e.g. <code className="text-cyan-400 font-mono">HTTP/401 Content-Security-Policy header is absent</code>).
            </li>
          </ul>

          <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 text-xs font-mono space-y-2 mt-2">
            <div className="flex items-center justify-between text-[10px] text-slate-400 border-b border-slate-800 pb-2">
              <span>TECHNICAL EVIDENCE TRACE SAMPLE</span>
              <span className="text-emerald-400">HTTP/401</span>
            </div>
            <pre className="text-cyan-300 bg-slate-950 p-2.5 rounded border border-slate-800 whitespace-pre-wrap">
...
// Missing Content-Security-Policy header in server response.`}
    ...
    4: {
      ...
      content: (
        <div className="space-y-4 text-xs text-slate-300">
          <p className="leading-relaxed">
            For every vulnerability, Saksham AI generates an instant <strong className="text-cyan-300">Express.js Security Snippet</strong> ready to paste into your backend code.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 font-mono text-[11px]">
            <div className="p-2.5 rounded-lg border border-slate-800 bg-slate-950">
              <span className="text-cyan-400 font-bold block mb-0.5">1. Copy Code Fix:</span>
              <span className="text-slate-400 font-sans text-xs">Click "Copy Code" and paste into your Express app.</span>
            </div>
            <div className="p-2.5 rounded-lg border border-slate-800 bg-slate-950">
              <span className="text-emerald-400 font-bold block mb-0.5">2. Click "Pass" or "Fail":</span>
              <span className="text-slate-400 font-sans text-xs">Run a retest to verify if the attack is blocked (<strong className="text-emerald-400 font-sans">PASSED</strong>) or still reproducible (<strong className="text-red-400 font-sans">FAILED</strong>).</span>
            </div>
          </div>
  ...
  contentSecurityPolicy: {
    ...
    5: {
      ...
      content: (
        <div className="space-y-4 text-xs text-slate-300">
          <p className="leading-relaxed">
            The <strong className="text-white font-mono">Activity</strong> stream records every event with timestamps, actor IDs, and color-coded event badges.
          </p>
          <ul className="space-y-2 list-disc pl-4 text-slate-300">
            <li>
              <strong className="text-cyan-300">Live Event Feed:</strong> Displays scan execution, retest results (<code className="text-red-400 font-mono">Retest FAILED</code> or <code className="text-emerald-400 font-mono">Retest PASSED</code>), and status updates.
            </li>
            <li>
              <strong className="text-cyan-300">Executive PDF & HTML Reports:</strong> Go to <strong className="text-white font-mono">Reports</strong> to download compliance reports for clients or stakeholders.
            </li>
          </ul>

          <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 text-xs font-mono space-y-2 mt-2">
            <span className="text-[10px] text-slate-400 uppercase tracking-wider block border-b border-slate-800 pb-2 font-bold">
            ...
            <div className="space-y-2 pt-1">
              <div className="flex items-center justify-between bg-slate-950 p-2 rounded border border-slate-800">
                <span className="inline-flex items-center gap-1.5 rounded bg-red-500/20 px-2 py-0.5 text-[10px] font-bold text-red-300 border border-red-500/30">
                  Retest FAILED
                </span>
                <span className="text-slate-400 text-[10px]">VUL-001 Missing CSP Header</span>
                <span className="text-slate-500 text-[10px]">14:52:58</span>
              </div>
              <div className="flex items-center justify-between bg-slate-950 p-2 rounded border border-slate-800">
                <span className="inline-flex items-center gap-1.5 rounded bg-emerald-500/20 px-2 py-0.5 text-[10px] font-bold text-emerald-300 border border-emerald-500/30">
                  Retest PASSED
                </span>
                <span className="text-slate-400 text-[10px]">VUL-002 Rate Limiting</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
          ...
          <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl border border-slate-800 bg-[#090f1f] p-4 sm:p-6 shadow-2xl z-10 space-y-4 sm:space-y-5">
            ...
            <div className="flex items-start justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-cyan-600/20 font-mono text-base font-bold text-cyan-400 border border-cyan-500/30">
                  {modalStep}
                </span>
                <div>
      ...
      <div className="space-y-6">
        ...
        <Card className={cn('border-slate-800 bg-[#090f1f] p-5 shadow-md space-y-4', activeStep === 1 && 'ring-1 ring-cyan-500/40')}>
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Step 1: Register Target API / Web Service</h3>
              <p className="text-xs text-slate-400">How to add a target project endpoint, select HTTP methods, and provide JSON payloads</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 font-mono text-xs space-y-2">
              <div className="flex items-center justify-between text-[10px] text-slate-400 border-b border-slate-800 pb-2">
                <span>TARGET REGISTRATION FORM SAMPLE</span>
                <span className="text-cyan-400">POST /api/v1/auth/login</span>
              </div>
  ...
  "role": "ADMIN"
        ...
        <Card className={cn('border-slate-800 bg-[#090f1f] p-5 shadow-md space-y-4', activeStep === 2 && 'ring-1 ring-cyan-500/40')}>
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Step 2: Automated Security Assessment & CVSS Scoring</h3>
              <p className="text-xs text-slate-400">What happens during scanning, what is CVSS 3.1, and how the Security Score is calculated</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 text-xs space-y-3">
              <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-slate-400 block border-b border-slate-800 pb-2">
                CVSS 3.1 SEVERITY SCALE BREAKDOWN
              </span>
              <div className="space-y-2 font-mono text-[11px]">
        ...
        <Card className={cn('border-slate-800 bg-[#090f1f] p-5 shadow-md space-y-4', activeStep === 3 && 'ring-1 ring-cyan-500/40')}>
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Step 3: Analyze Findings & Technical PoC Evidence</h3>
              <p className="text-xs text-slate-400">How to inspect vulnerability details, copy reproducible cURL PoCs, and view HTTP evidence</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            ...
            <div className="rounded-xl border border-slate-800 bg-[#040814] p-4 text-xs font-mono space-y-2">
              <div className="flex items-center justify-between text-[10px] text-slate-400 border-b border-slate-800 pb-2">
                <span>TECHNICAL EVIDENCE TRACE SAMPLE</span>
                <span className="text-emerald-400">HTTP/401</span>
              </div>
...
// Missing Content-Security-Policy header in server response.`}
        ...
        <Card className={cn('border-slate-800 bg-[#090f1f] p-5 shadow-md space-y-4', activeStep === 4 && 'ring-1 ring-cyan-500/40')}>
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Step 4: Express Code Fix Snippets & Retesting</h3>
              <p className="text-xs text-slate-400">How to copy ready-to-use Express.js code fixes and test if the vulnerability is fixed</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
            <div className="space-y-3 text-xs text-slate-300 leading-relaxed">
              ...
              <div className="space-y-2 font-mono text-[11px]">
                <div className="p-2.5 rounded-lg border border-slate-800 bg-slate-950">
                  <span className="text-cyan-400 font-bold block mb-0.5">1. Copy Code Fix:</span>
                  <span className="text-slate-400 font-sans text-xs">Click the "Copy Code" button and paste into your Express app.</span>
                </div>
                <div className="p-2.5 rounded-lg border border-slate-800 bg-slate-950">
                  <span className="text-emerald-400 font-bold block mb-0.5">2. Click "Pass" or "Fail":</span>
                  <span className="text-slate-400 font-sans text-xs">Run a retest to verify if the attack is blocked (<strong className="text-emerald-400 font-sans">PASSED</strong>) or still reproducible (<strong className="text-red-400 font-sans">FAILED</strong>).</span>
      ...
      scriptSrc: ["'self'"]
        ...
        <Card className={cn('border-slate-800 bg-[#090f1f] p-5 shadow-md space-y-4', activeStep === 5 && 'ring-1 ring-cyan-500/40')}>
          <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
            ...
            <div>
              <h3 className="text-base font-bold text-white">Step 5: Activity Audit Log Stream & Executive Reports</h3>
              <p className="text-xs text-slate-400">Track all security scan events, status updates, and download PDF audit reports</p>
            </div>
          </div>
          ...
          {filteredGlossary.map((item, idx) => (
            <div key={idx} className="rounded-xl border border-slate-800 bg-slate-950/80 p-4 space-y-2 shadow-sm transition hover:border-slate-700 hover:shadow-md">
              ...
              <p className="text-xs text-slate-200">
                <span className="text-cyan-400 font-mono font-semibold">Technical: </span>
                {item.tech}
              </p>
              <p className="text-xs text-slate-300 border-t border-slate-800/80 pt-1.5 mt-1.5">
