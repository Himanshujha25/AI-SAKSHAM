    ...
    <div>
      ...
      <div className="flex flex-col gap-2">
        {(list.data?.assessments || []).map((a) => (
          <Link key={a._id} to={`/assessments/${a._id}`} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
...
export function AssessmentDetail() {
  ...
  return (
    <div>
      <PageHeader title={`${assessment.type} Assessment`} subtitle={`Status: ${assessment.status} · Score: ${assessment.summary?.securityScore ?? '—'}/100`} />
      <Card className="mb-4">
        ...
        <ul className="grid gap-1 text-sm md:grid-cols-2">
          {STAGES.map((s) => (
            <li key={s} className="flex justify-between rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800">
              <span>{s}</span><span>{assessment.progress?.[s] === 'done' ? '✓' : assessment.progress?.[s] === 'running' ? '●' : '○'}</span>
            </li>
          ))}
        </ul>
      </Card>
