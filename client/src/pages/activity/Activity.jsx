...
export function Activity() {
  ...
  const getEventStyle = (action = '') => {
    ...
    if (act.includes('started') || act.includes('queued')) {
      return {
        icon: Zap,
        badgeColor: 'border-blue-200 dark:border-cyan-500/30 bg-blue-50 dark:bg-cyan-500/10 text-blue-600 dark:text-cyan-400',
        dotBg: 'bg-cyan-500',
    ...
    return {
      icon: ActivityIcon,
      badgeColor: 'border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300',
      dotBg: 'bg-slate-400',
  ...
  return (
    <div className="relative min-h-screen space-y-6 pb-16">
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] px-3 py-1.5 font-mono text-[11px] font-bold text-slate-600 dark:text-slate-300 shadow-sm">
              <ActivityIcon size={13} className="text-emerald-700 dark:text-emerald-300" /> {metrics.total} Events
            </span>
            <span className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 dark:border-cyan-500/30 bg-white dark:bg-gradient-to-r dark:from-cyan-950/60 dark:to-slate-900 px-3 py-1.5 font-mono text-[11px] font-bold text-[#0f1f3d] dark:text-white shadow-md">
              <span className="live-dot relative inline-block h-1.5 w-1.5 rounded-full bg-emerald-400 text-emerald-600 dark:text-emerald-400" /> LIVE FEED
            </span>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-4 md:p-5 shadow-md">
          <div className="flex items-center justify-between">
            <PremiumIcon icon={ScrollText} tone="cyan" size="md" />
            <span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-blue-600 dark:text-cyan-300">All logs</span>
          </div>
          <div className="mt-3">
            <span className="font-mono text-2xl font-extrabold tracking-tight text-[#0f1f3d] dark:text-white">{metrics.total}</span>
            <p className="mt-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Logged Events</p>
          </div>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-4 md:p-5 shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-700 dark:text-emerald-300">Audits</span>
          </div>
          <div className="mt-3">
            <span className="font-mono text-2xl font-extrabold tracking-tight text-[#0f1f3d] dark:text-white">{metrics.scans}</span>
            <p className="mt-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Scans Executed</p>
          </div>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-4 md:p-5 shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-purple-700 dark:text-purple-300">Reports</span>
          </div>
          <div className="mt-3">
            <span className="font-mono text-2xl font-extrabold tracking-tight text-[#0f1f3d] dark:text-white">{metrics.reports}</span>
            <p className="mt-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Reports Exported</p>
          </div>
        </Card>
        ...
        <Card className="border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-4 md:p-5 shadow-md">
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-blue-700 dark:text-blue-300">Workspaces</span>
          </div>
          <div className="mt-3">
            <span className="font-mono text-2xl font-extrabold tracking-tight text-[#0f1f3d] dark:text-white">{metrics.projectsCount}</span>
            <p className="mt-0.5 font-mono text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Project Operations</p>
          </div>
        </Card>
      </div>

      {/* Filter & Search Bar Card */}
      <Card className="relative z-20 border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-3 md:p-4 shadow-md">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
          ...
          <div className="relative w-full md:flex-1 min-w-0">
            ...
            <input
              ...
              className="w-full rounded-md border border-slate-200 dark:border-slate-700/80 bg-white dark:bg-slate-900/90 min-h-[44px] py-2 pl-9 pr-8 text-xs text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-600 focus:border-cyan-500/50 focus:outline-none focus:ring-1 focus:ring-cyan-500/50"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery('')}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 hover:text-[#0f1f3d] dark:hover:text-white p-1"
      ...
      {!isLoading && !isError && filteredEvents.length > 0 && (
        <div className="relative space-y-4 pl-6 before:absolute before:left-3.5 before:top-3 before:h-[calc(100%-1.5rem)] before:w-0.5 before:bg-slate-100 dark:bg-slate-800">
            ...
            return (
              <div key={e._id} className="relative group">
                ...
                <span className={`absolute -left-6 top-4 flex h-6 w-6 items-center justify-center rounded-lg border border-white/20 ${style.dotBg} ring-1 ring-black/20 transition-transform duration-200 group-hover:scale-110 z-10`}>
                  <Icon size={12} strokeWidth={2.4} className="text-[#0f1f3d] dark:text-white" />
                </span>

                {/* Card Container */}
                <div
                  onClick={() => setSelectedEvent(e)}
                  className="cursor-pointer rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-4 shadow-md transition duration-200 hover:border-blue-200 dark:border-cyan-500/40 hover:shadow-lg"
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {projectObj && (
                          <span className="inline-flex items-center gap-1 rounded-md border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900 px-2 py-0.5 font-mono text-[10px] font-semibold text-slate-600 dark:text-slate-300">
                            <FolderKanban size={10} className="text-blue-600 dark:text-cyan-400" />
                            {projName}
                          </span>
                        )}
                      </div>
                      ...
                      {/* Detail Message */}
                      <p className="text-sm font-bold tracking-tight text-slate-900 dark:text-slate-100 leading-snug">
                        {e.detail || e.action}
                      </p>

                      {/* Detailed Meta Sub-row */}
                      <div className="flex flex-wrap items-center gap-3 pt-1 font-mono text-[11px] text-slate-500 dark:text-slate-400">
                        <span className="flex items-center gap-1 text-slate-600 dark:text-slate-300">
                          <User size={12} className="text-blue-600 dark:text-cyan-400" />
                          {actorName}
                          <span className="rounded bg-slate-100 dark:bg-slate-800 px-1.5 py-0.2 text-[9px] font-bold text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                            {actorRole}
                          </span>
                        </span>
                    ...
                    <div className="flex items-center gap-1 font-mono text-[11px] font-bold uppercase tracking-wider text-slate-500 transition group-hover:text-blue-600 dark:group-hover:text-cyan-300">
                      <span>Inspect</span>
        ...
        {selectedEvent && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm">
            <motion.div
              ...
              className="relative w-full max-w-lg overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-[#090f1f] p-6 shadow-2xl space-y-4"
            >
              <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                <div className="flex items-center gap-2.5 font-bold tracking-tight text-slate-900 dark:text-slate-100 text-base">
                  <PremiumIcon icon={ActivityIcon} tone="cyan" size="sm" />
                  Security Event Audit Record
                </div>
                <button
                  onClick={() => setSelectedEvent(null)}
                  className="rounded-lg p-1 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 hover:text-[#0f1f3d] dark:hover:text-white"
                >
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="space-y-3 font-mono text-xs">
                <div>
                  ...
                  <span className="text-blue-600 dark:text-cyan-400 font-bold text-sm">{selectedEvent.action}</span>
                </div>

                <div>
                  <span className="text-[10px] text-slate-500 uppercase tracking-wider block">Execution Detail</span>
                  <span className="text-slate-700 dark:text-slate-200">{selectedEvent.detail || 'No extra detail provided.'}</span>
                </div>

                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 p-2.5">
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block">Actor / Trigger</span>
                    <span className="text-slate-600 dark:text-slate-300 font-bold">{selectedEvent.actor?.name || 'System / Auto-Scanner'}</span>
                  ...
                  <div className="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/80 p-2.5">
                    ...
                    <span className="text-slate-600 dark:text-slate-300 font-bold">{new Date(selectedEvent.createdAt).toLocaleDateString()}</span>
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-200 dark:border-slate-800">
                <button
                  onClick={() => setSelectedEvent(null)}
                  className="rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 px-4 py-2 text-xs font-bold text-slate-700 dark:text-slate-200 transition"
                >
                  Close
                </button>
              </div>
